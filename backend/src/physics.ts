...
// Physics update function (identical on client and server)
...
export function updatePlayerPhysics(
  ...
  const { GRAVITY, THRUST_POWER, ROTATION_ACCEL, ANGULAR_DRAG, DRAG } = PHYSICS_CONSTANTS;

  // Apply rotational physics
  if (input.rotateLeft) player.rv -= ROTATION_ACCEL;
  if (input.rotateRight) player.rv += ROTATION_ACCEL;
...
export function checkBoundaryCollision(
  ...
  const { PLAYER_RADIUS } = PHYSICS_CONSTANTS;

  return (
    player.x - PLAYER_RADIUS < 0 ||
...
export function handlePlatformCollision(
  ...
  const { BOUNCE, PLAYER_RADIUS } = PHYSICS_CONSTANTS;
